...
namespace LiteDB.Studio.Wpf.Tests.UI
{
    public class MainWindowXamlTests
    {
        ...
        public async Task MainWindow_Xaml_ShouldContain_F5KeyBinding()
        {
            ...
            while (dir != null)
            {
                var path = Path.Combine(dir.FullName, "..", "..", "..", "LiteDB.Studio.Wpf", "MainWindow.xaml");
        ...
        public async Task MainWindow_Xaml_ShouldEnable_AvalonEditCompletion()
        {
            ...
            Assert.True(text.Contains("controls:AvalonEditBehaviors.EditorText") || text.Contains("controls:AvalonEditBehaviors.ShowCompletionCommand"), "MainWindow.xaml should bind AvalonEdit to the MVVM behaviors (EditorText or ShowCompletionCommand)");
        }
    }
}
