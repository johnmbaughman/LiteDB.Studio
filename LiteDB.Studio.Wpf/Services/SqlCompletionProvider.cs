...
/// </summary>
public class SqlCompletionProvider(string text, string? description = null, object? tag = null)
    : ICompletionData
{
    /// <summary>
    ...
    /// </summary>
    public static async Task<IEnumerable<SqlCompletionProvider>>
        GetKeywordCompletionsFromFileAsync(string? filePath = null, CancellationToken cancellationToken = default)
    {
        var path = filePath;
        var customProvided = !string.IsNullOrWhiteSpace(filePath);
        if (string.IsNullOrWhiteSpace(path))
        {
            path = System.IO.Path.Combine(AppContext.BaseDirectory, DefaultKeywordsRelativePath.Replace('/', System.IO.Path.DirectorySeparatorChar));
        }

        var list = new List<SqlCompletionProvider>();
        ...
        list.AddRange(from kw in keywords where !string.IsNullOrWhiteSpace(kw) select FromKeyword(kw.Trim()));
