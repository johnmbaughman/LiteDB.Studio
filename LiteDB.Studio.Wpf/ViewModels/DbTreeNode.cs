...
namespace LiteDB.Studio.Wpf.ViewModels
{
    public partial class DbTreeNode : ObservableObject
    {
        ...
        public DbTreeNode(IDatabaseService databaseService, Action<string>? insertSnippetAction = null, Func<string, bool>? confirmer = null)
        {
            ...
            _insertSnippetAction = insertSnippetAction;
        ...
        private async Task DropAsync()
        {
            ...
            var confirmed = _confirmer?.Invoke(message) ?? (MessageBox.Show(message, "Confirm Drop", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes);
            if (!confirmed) return;
